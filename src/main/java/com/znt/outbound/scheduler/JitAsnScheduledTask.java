...
/**
 * JIT ASN 排程任務
 * 每5分鐘自動執行JIT ASN資料撈取並調用API
 */
@Component
@RequiredArgsConstructor
@Slf4j
public class JitAsnScheduledTask {

    private final JitAsnMappingService jitAsnMappingService;

    /**
     * 排程任務：每5分鐘執行一次JIT ASN處理
     * Cron: 秒 分 時 日 月 週
    ...
    public void executeJitAsnProcessing() {
        ...
        try {
            // 調用JIT ASN處理服務
            log.info("[{}] 開始調用 JIT ASN 處理服務...", taskId);
    ...
    private void handleScheduledTaskError(String taskId, Exception exception) {
        try {
            ...
            // 確保錯誤處理本身不會拋出異常
            log.error("[{}] 處理排程任務錯誤時發生異常", taskId, handlingException);
        }
    }
    ...
    private void analyzeExceptionAndSuggest(String taskId, Exception exception) {
        ...
        switch (exceptionType) {
            ...
            case "DataAccessException":
                suggestion = "資料庫連線異常，請檢查資料庫狀態和連線設定";
                break;
            case "ConnectException":
            case "SocketTimeoutException":
                suggestion = "網路連線異常，請檢查外部API服務狀態";
                break;
            case "NullPointerException":
                suggestion = "空指標異常，請檢查資料完整性";
                break;
    ...
    private void logErrorStatistics(String taskId, Exception exception) {
        ...
        // - 錯誤類型分布
        // - 錯誤趨勢分析
        log.debug("[{}] 提示：可考慮實作錯誤統計持久化功能", taskId);
    }
    ...
    private void logScheduleStart(String taskId) {
        log.info("=".repeat(80));
        log.info("[{}] JIT ASN 排程任務開始執行", taskId);
    ...
    private void logScheduleSuccess(String taskId, long executionTime) {
        log.info("=".repeat(80));
        log.info("[{}] JIT ASN 排程任務執行成功", taskId);
        ...
        if (executionTime > 60000) { // 超過1分鐘
            log.warn("[{}] 注意：執行時間較長，建議檢查系統效能", taskId);
    ...
    private void logScheduleFailure(String taskId, long executionTime, Exception exception) {
        log.error("=".repeat(80));
        log.error("[{}] JIT ASN 排程任務執行失敗", taskId);

...
BEGIN
    ...
    FROM (
        SELECT 
            ...
            c.DELIVERY_ADDRESS AS INVOICE_ADDRESS,
            '' AS SHIP_TO_NO,
            c.DELIVERY_ADDRESS AS DELIVERY_ADDRESS,
            c.RECIPIENT_NAME AS SHIP_TO_CONTACT,
            ...
            '' AS INV_AND_PAC,
            '' AS CUST_PART_NO2,
            NVL(NVL(c.VENDOR_MATERIAL_INFO, ZEN_GET_WMS_ITEM_F(c.PARTNO)), c.PARTNO) AS PO_REMARK, -- 原廠來料訊息
            '' AS RMA_NUMBER,
            '0' AS ORD_TYPE,
            'TY' AS QC_TYPE
        FROM 

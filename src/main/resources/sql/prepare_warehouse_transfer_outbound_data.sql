...
FROM (
         ...
         SELECT
             ...
             CASE
                 WHEN ZMTH.HEADER_ID IS NULL
                     ...
                     OR ZEN_GET_BRAND_ALIAS_F@ZENPROD(MSIB.ORGANIZATION_ID, ZMTL.INVENTORY_ITEM_ID) IS NULL
            ...
            ZMTH.SHIP_TO_PHONE AS SHIP_TO_PHONE,
            ...
            '' AS INV_AND_PAC,
            '' AS CUST_PART_NO2,
            NVL(NVL(ZMTL.vendor_material_info, ZEN_GET_WMS_ITEM_F(MSIB.SEGMENT1,ZMTL.ORGANIZATION_ID)), MSIB.SEGMENT1) AS PO_REMARK,
            '' AS RMA_NUMBER,
            DECODE(ZMTL.ORGANIZATION_ID,169,'ZSH','SHC') AS ORD_TYPE,
            'TY' AS QC_TYPE
         FROM
             ZEN_MTL_TXN_REQUEST_HEADERS@ZENPROD ZMTH
             INNER JOIN ZEN_MTL_TXN_REQUEST_LINES@ZENPROD ZMTL ON ZMTH.HEADER_ID = ZMTL.HEADER_ID
             INNER JOIN MTL_SYSTEM_ITEMS_B@ZENPROD MSIB ON MSIB.INVENTORY_ITEM_ID = ZMTL.INVENTORY_ITEM_ID
             AND MSIB.ORGANIZATION_ID = ZMTH.ORGANIZATION_ID
             INNER JOIN FND_USER@ZENPROD FU ON ZMTH.CREATED_BY = FU.USER_ID
             INNER JOIN MFG_LOOKUPS@ZENPROD MFGL ON MFGL.LOOKUP_TYPE = 'MTL_TXN_REQUEST_STATUS'
             AND MFGL.LOOKUP_CODE = ZMTL.LINE_STATUS
             INNER JOIN ZEN_ORGANIZATION_DEFINITIONS_V@ZENPROD OOD ON ZMTH.ORGANIZATION_ID = OOD.ORGANIZATION_ID
             LEFT JOIN apps.mtl_item_locations@ZENPROD mil ON mil.organization_id = ZMTL.organization_id
             ...
             JOIN HR_EMPLOYEES@ZENPROD e ON u.USER_NO = e.ATTRIBUTE1
             JOIN FND_USER@ZENPROD f ON e.EMPLOYEE_ID = f.EMPLOYEE_ID
             ) dep ON FU.USER_NAME = dep.USER_NAME
             INNER JOIN MTL_MATERIAL_TRANSACTIONS@ZENPROD mmt ON mmt.transaction_type_id = 2
             ...
             LEFT JOIN apps.mtl_item_locations@ZENPROD mil1 ON ZMTL.organization_id = mil1.organization_id
         ...
         FROM
             ...
             AND ZMTL.TO_LOCATOR_ID = mil1.inventory_location_id
         WHERE ZMTL.ORGANIZATION_ID = 371
           AND ZMTL.LINE_STATUS = 5  -- CLOSED
           AND ZMTH.HEADER_STATUS = 3  -- Approved
           AND ZMTL.FROM_SUBINVENTORY_CODE = '外存倉'
           AND (mil.segment1 LIKE '%基通%' OR mil.segment2 LIKE '%基通%')
